...
sim_ideology <- function(){
  ...
  #gets two-param values
  two_param <- mirt(complete_small, model = 1, itemtype = "2PL", se = TRUE)
  two_param_vals <- fscores(two_param)
  ...
  closest_vals_two_param <- two_param_vals[vec_of_differences == min_distance]
  vec_of_ideologies_two_param <- sapply(closest_vals_two_param, function(x) ecdf(two_param_vals)(x))
  
  #returning a dataframe of the means of both ideologies
  return (data.frame(mean(vec_of_ideologies_wnom), mean(vec_of_ideologies_two_param)))
}
